...
export function CourseDetail({ courseID }: { courseID: any }) {
  const { data } = useCourseData(courseID);
  const sampleData = {
    students: 123456,
    rating: 4.5,
    reviews: 123456,
    lastUpdated: data?.updatedAt ?? "new Date().toDateString()",
    language: "English",
  };
  return (
    <article
      id="content"
      className="flex flex-grow flex-col h-[260px] max-w-full gap-3 py-5"
    >
      ...
      <div className="flex flex-row gap-3 mt-auto items-center">
        <img
          src={data?.authorInfo?.imageUrl}
          className="rounded-full bg-black w-8 aspect-square object-cover border-[2px] border-black"
        ></img>
