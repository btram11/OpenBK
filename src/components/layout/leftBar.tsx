...
const LeftBar: React.FC = () => {
  ...
  const baseLinks = [
    ...
    {
      href: `/${roleString(user?.role)?.toLowerCase()}/profile`,
      label: "My Profile",
    },
    {
      href: `/${roleString(user?.role)?.toLowerCase()}/setting`,
      label: "Settings",
    },
  ];
  ...
  const roleSpecificLinks =
    user?.role === "LEARNER"
      ? [
          ...
          {
            href: `/${roleString(user?.role)?.toLowerCase()}/quiz-attempts`,
            label: "My Quiz Attemps",
          },
        ]
      : [
          {
  ...
  return (
    <nav className="flex flex-col basis-1/5 p-8 gap-4 rounded-l-2xl border-[1.5px] border-[#E3E2E6] bg-white w-fit h-fit">
      <div className="text-black font-medium text">Welcome {user?.name}</div>
