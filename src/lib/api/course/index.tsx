...
const updateCourse = async (id: number, courseName: string, description: string) => {
  try {
    const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/course/${id}`, {
      courseName,
      description
    });
...
const deleteCourse = async (id: number) => {
  try {
    const res = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/course/${id}`);
