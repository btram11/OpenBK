...
const FilterSection: React.FC<{
    ...
    return (
        <div className="flex overflow-hidden flex-col justify-center p-8 max-w-full bg-gray-100 rounded-3xl w-[260px] max-md:px-5">
            <div className="self-start text-xl font-bold leading-none text-black">{title}</div>
            <div className="flex overflow-hidden flex-col mt-5 w-full">
                <RadioGroup className="flex overflow-hidden flex-col items-start w-full text-base text-black">
                    {items.map((item) => (
                        <div key={item.id} className="flex overflow-hidden text-m gap-2 items-center first:mt-0">
...
const Pagination: React.FC<{
    ...
    return (
        <nav className="flex gap-2.5 justify-between items-center text-base tracking-wide text-center text-neutral-950" role="navigation" aria-label="Pagination">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                    key={page}
                    onClick={() => onPageChange(page)}
                    className={`self-stretch px-0.5 my-auto h-[33px] min-h-[33px] rounded-[30px] w-[33px] border border-black border-solid focus:outline-none focus:ring-2 focus:ring-sky-600 ${currentPage === page ? 'text-white bg-sky-600' : 'bg-gray-100'
                        }`}
